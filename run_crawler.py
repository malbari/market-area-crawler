        ...
        if all_articles:
            ...
            for article in all_articles:
                articles_data.append({
                    'title': article.title,
                    'url': article.url,
                    'market_areas': article.market_areas,
...
def print_summary(report):
    ...
    if report['markets']:
        ...
        for market, data in report['markets'].items():
            trend_emoji = "📈" if data['sentiment_trend'] == 'positive' else "📉" if data['sentiment_trend'] == 'negative' else "➡️"
            print(f"  {trend_emoji} {market.upper()}: {data['average_sentiment']:.1f}/100 ({data['article_count']} articoli)")
    
    if report['locations']:
        print(f"\n🌍 SENTIMENT PER LOCALITÀ:")
        for location, data in report['locations'].items():
            trend_emoji = "📈" if data['sentiment_trend'] == 'positive' else "📉" if data['sentiment_trend'] == 'negative' else "➡️"
            print(f"  {trend_emoji} {location.upper()}: {data['average_sentiment']:.1f}/100 ({data['article_count']} articoli)")
    
    if report['market_location_matrix']:
        print(f"\n🎯 TOP CORRELAZIONI MERCATO-LOCALITÀ:")
        # Ordina per sentiment score
        sorted_matrix = sorted(
            report['market_location_matrix'].items(),
        ...
        for i, (key, data) in enumerate(sorted_matrix[:5]):  # Top 5
            trend_emoji = "📈" if data['sentiment_trend'] == 'positive' else "📉" if data['sentiment_trend'] == 'negative' else "➡️"
            print(f"  {i+1}. {trend_emoji} {data['market'].upper()} in {data['location'].upper()}: {data['average_sentiment']:.1f}/100")
